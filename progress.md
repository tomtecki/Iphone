...
Ostatnia aktualizacja: 2026-09-23 (wyłączenie czatbota i zakładki „Pomoc” na życzenie klienta)
...
- **Naprawiona analityka Umami (2026-08-25): zła domena skryptu.** `analytics.mojiphone.pl` wpięta w kod strony w ogóle się nie rozwiązywała (DNS) — zweryfikowane `curl -I`. Strona jest obecnie faktycznie serwowana pod `iphone.showflow.pl`, nie pod `mojiphone.pl` (patrz odkrycie niżej), a instancja Umami jest dostępna pod `analytics.iphone.showflow.pl` (zweryfikowane `curl -I` → 200), z tym samym `data-website-id`. Zmieniono `src` skryptu we wszystkich 4 plikach HTML (`index.html`, `sprawdz-model-iphone.html`, `problemy-z-iphone.html`, `polityka-prywatnosci.html`). **Wymaga też ręcznej zmiany w panelu Umami:** domena strony (Ustawienia → Strony internetowe) na `iphone.showflow.pl` — inaczej Umami może odrzucać zdarzenia mimo poprawnie ładującego się skryptu. To poprawka tymczasowa, dopasowana do obecnego stanu wdrożenia — po docelowym spięciu z `mojiphone.pl` trzeba to zmienić ponownie (szczegóły: `analityka.md`).
- **Poprawa czatbota (2026-08-25):** dopracowanie rozmowy — bot rozpoznaje teraz powitania ("cześć"/"hej", od razu proponuje listę modeli), podziękowania ("dzięki") i pytanie "czy jesteś botem" (jawnie przyznaje, że to automatyczny asystent bez AI, kieruje do telefonu); rozbudowany fallback — zamiast samego "zadzwoń albo napisz formularz" bot wymienia, z czym może pomóc. **Nowy temat: zamiennik vs oryginał** (priorytet klienta) — dodany do bota i jako nowa pozycja FAQ na `index.html` (widoczna + w schemacie `FAQPage`), jedno źródło treści dla obu. **Śledzenie skuteczności bota w Umami** — nowe zdarzenia `bot_rule_matched`, `bot_part_asked`, `bot_model_missing`, `bot_problem_matched`, `bot_generation_ambiguous`, `bot_onboarding_step`, `bot_fallback` (bez treści wiadomości klienta), domyka rekomendację Architekta z `pomysły.md` sekcja 7 — dotąd wiadomo było tylko co klient ogląda, nie co pyta bota i gdzie bot nie ma odpowiedzi. Zweryfikowane lokalnie uruchomieniem `script.js` w Node. Szczegóły: `bot.md`, `bot-baza-pytan.md`.

- **Wyłączony czatbot i zakładka „Pomoc” (2026-09-23, decyzja klienta).** Widget czatu w `index.html` zakomentowany w całości — cała logika bota (`BOT_*`, `handleBotQuestion`, obsługa panelu) zostaje nietknięta w `script.js` i sama się nie uruchamia, bo inicjalizacja jest za strażnikiem `if (chatWidget && chatToggle && chatPanel)`. Nie działa więc też auto-otwarcie po 3 s ani zdarzenia `bot_*` w Umami. Link „Pomoc” w nawigacji zakomentowany na `index.html`, `sprawdz-model-iphone.html` i `problemy-z-iphone.html` — sama podstrona `problemy-z-iphone.html` nadal istnieje, jest dostępna pod bezpośrednim adresem i **zostaje w `sitemap.xml`** (nie było polecenia jej odindeksowywać; do rozważenia, jeśli ma zniknąć z wyników Google na czas rozbudowy). Przywrócenie jednego i drugiego to odkomentowanie tych bloków.
